...
-- App-level review/source columns (Phase 2 extension)
alter table if exists story_nodes
  add column if not exists review_status text check (review_status in ('approved','pending','needs_source')) default 'pending',
  add column if not exists source_status text check (source_status in ('cited','pending','none')) default 'pending';

alter table if exists story_events
  add column if not exists review_status text check (review_status in ('approved','pending','needs_source')) default 'pending',
  add column if not exists source_status text check (source_status in ('cited','pending','none')) default 'pending';
