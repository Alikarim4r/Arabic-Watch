...
function validateDecision(entry, i) {
  ...
  const decision = str(entry.scholar_decision);
  ...
  if (decision === 'split_event' && !str(entry.scholar_note)) {
    warnings.push(`${prefix}: split_event بدون scholar_note — يُفضّل توثيق تقسيم الحدث`);
  }

  if (decision === 'approve_after_source_check') {
    if (!canProposeApproved(entry, entry)) {
      warnings.push(
        `${prefix}: approve_after_source_check — شروط approved غير مكتملة؛ يُبقى pending`
      );
    }
  }
...
function buildBaseMapping(entry) {
  return {
    ...
    scholar_decision: str(entry.scholar_decision) || 'undecided',
    scholar_note: str(entry.scholar_note),
    proposed_title_ar: '',
...
function compileEntry(entry) {
  ...
  if (decision === 'reject_mapping') {
    rejectedProposals.push({
      event_id: entry.event_id,
      scholar_decision: decision,
      scholar_note: str(entry.scholar_note),
      original_surah_id: entry.surah_id,
      original_ayah_from: entry.ayah_from,
      original_ayah_to: entry.ayah_to,
      reviewer_note: str(entry.reviewer_note),
    });
    return null;
  }
  ...
  switch (decision) {
    ...
    case 'approve_after_source_check':
      if (canProposeApproved(entry, mapping)) {
  ...
  if (mapping.proposed_review_status === 'approved' && !canProposeApproved(entry, mapping)) {
    mapping.proposed_review_status = 'pending';
    mapping.compile_status = 'approve_blocked';
  }
...
if (template.meta?.status !== 'review_template') {
  errors.push('input meta.status يجب أن يكون review_template');
}
...
if (errors.length) {
  console.log('FAIL compile_scholar_review_decisions', inputPath);
  errors.forEach((e) => console.log(' ', e));
  process.exit(1);
}

const approvedCount = mappings.filter((m) => m.proposed_review_status === 'approved').length;
if (approvedCount > 0 && mappings.some((m) => m.compile_status === 'approve_blocked')) {
  warnings.push('some items blocked from auto-approval');
}
...
const output = {
  meta: {
    ...
    batch_id: template.meta?.batch_id || 'evidence_mapping_sprint_01',
    description_ar:
      'مسودة مُجمّعة من قرارات العالم — ليست محتوى معتمدًا. لا تُطبّق تلقائيًا على seed أو Supabase.',
    ...
    summary: {
      ...
      undecided: mappings.filter((m) => m.scholar_decision === 'undecided').length,
...
console.log('PASS compile_scholar_review_decisions');
